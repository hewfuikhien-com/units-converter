export class Converter {
	...
	to(to: string) {
		...
		if (this.origin.abbr === this.destination.abbr) {
			return Object.assign(
				{ value: this.negative ? this.val * -1 : this.val },
		...
		let value = result / this.destination.unit.to_anchor;
		return Object.assign({ value: this.negative ? value * -1 : value }, this.describe(this.destination.abbr));
	}
	...
	toBest(options?: any) {
		...
		return this.list()
			.filter(
				(item) => !options.exclude.includes(item.unit) && this.describe(item.unit).system === this.origin.system
			)
			.reduce((acc: any, item) => {
				const result = this.to(item.unit);
				if (
					!acc ||
					(Math.abs(result.value) >= options.cutOffNumber && Math.abs(result.value) < Math.abs(acc.value))
				) {
					return result;
				} else {
					return acc;
